...
export const NutritionistProfile: React.FC<NutritionistProfileProps> = ({ data, onSave, onResetData, onLogout }) => {
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/3">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 flex flex-col items-center text-center h-full">
            ...
            <div className="w-full pt-6 border-t border-slate-100 dark:border-slate-700 mt-auto space-y-3">
                <div className="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider font-bold mb-3">Ações da Conta</div>
                
                {onLogout && (
                    <button 
                        onClick={onLogout}
                        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors text-sm font-medium border border-slate-200 dark:border-slate-700"
                    >
                        <LogOut size={16} /> Sair
                    </button>
                )}
                ...
                {onResetData && (
                    <button 
                        onClick={onResetData}
                        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors text-sm font-medium border border-rose-100 dark:border-rose-900/30"
                    >
