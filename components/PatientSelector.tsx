...
export const PatientSelector: React.FC<PatientSelectorProps> = ({ 
    ...
    return (
      <div className="max-w-5xl mx-auto mt-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl font-bold text-slate-800 mb-2">O que você deseja fazer?</h2>
          <p className="text-slate-500">Escolha uma ação para iniciar.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">Nova Avaliação</h3>
            <p className="text-slate-500 text-center text-sm leading-relaxed">
          ...
          <button 
            ...
            className="flex flex-col items-center p-8 bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl hover:border-orange-500 hover:-translate-y-1 transition-all group"
          >
            <div className="w-20 h-20 bg-orange-50 text-orange-600 rounded-full flex items-center justify-center mb-6 group-hover:bg-orange-600 group-hover:text-white transition-colors">
              <Utensils size={36} />
            </div>
            <h3 className="text-xl font-bold text-slate-800 mb-2">Nova Dieta</h3>
            <p className="text-slate-500 text-center text-sm leading-relaxed">
  ...
  const actionColor = isDiet ? 'text-orange-600' : 'text-blue-600';
  const actionBg = isDiet ? 'bg-orange-50' : 'bg-blue-50';
        ...
        <div className="p-6">
            <div className="relative mb-6">
                ...
                <input 
                  ...
                  className={`w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 ${actionBorder} outline-none transition-all text-lg`}
                />
            </div>
                ...
                {filteredPatients.length === 0 ? (
                  ...
                  filteredPatients.map(patient => (
                    <button
                      ...
                      className="w-full flex items-center gap-4 p-4 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-200 transition-all text-left group"
                    >
                      ...
                      <div className="flex-1">
                        <h4 className="font-bold text-slate-700 text-lg group-hover:text-slate-900">{patient.name}</h4>
                        <p className="text-slate-500 text-sm">{patient.profession || 'Sem profissão'}</p>
                      </div>
